...
class BallTracker:
    ...
    def interpolate_ball_positions(self, ball_detections):
        ball_positions = [bbox.get(1, []) for bbox in ball_detections]
        ...
        ball_positions = [{1: bbox} for bbox in df_ball_positions.to_numpy().tolist()]
        
        return ball_positions
    
    def get_ball_shot_frames(self, ball_positions):
        ball_positions = [bbox.get(1, []) for bbox in ball_positions]
        df_ball_positions = pd.DataFrame(ball_positions, columns=['x1', 'y1', 'x2', 'y2'])
        ...
        for i in range(1, len(df_ball_positions) - int(min_change_frames_for_hit * 1.2)):
            ...
            if negative_position_change or positive_position_change:
                ...
                if change_count >= min_change_frames_for_hit:
                    df_ball_positions['ball_hit'].iloc[i] = 1

        frame_nums_with_ball_hits = df_ball_positions[df_ball_positions['ball_hit'] == 1].index.tolist()
        return frame_nums_with_ball_hits
        
    def detect_frames(self, frames, read_from_stub: bool = False, stub_path: str = None):
        ball_detections = []
    ...
    def draw_bboxes(self, video_frames, player_detections):
        ...
        for frame, ball_dict in zip(video_frames, player_detections):
            for track_id, bbox in ball_dict.items():
                ...
                cv2.putText(frame, f"Ball {track_id}", (int(x1), int(y1 - 10)), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (0, 255, 255), 2)
